...
def POC_model(shapes, p):
    ...
    last_layers = [Reshape([last_shapes[i][1]*last_shapes[i][2]])(last_layers[i]) for i in range(len(last_layers))]
    X = Concatenate(axis=1)(last_layers)
    
    '''the model we add on top of preloaded level 1 models'''
    X = Dense(int(p['dense1']),activation='relu',kernel_initializer='he_uniform')(X)
    X = BatchNormalization()(X)
    X = Dropout(float(p['dropout1']))(X)
    
    X = Dense(int(p['dense2']),activation='relu',kernel_initializer='he_uniform')(X)
    X = BatchNormalization()(X)
