...
def Params():
    ...
    params = {
        ...
        'dropout3': (0, 1),
        'dropoutL': (0, 1),
        'gru_size2': [32, 64, 128, 256],
        'gru_size3': [32, 64, 128, 256],
...
def POC_model(shapes, p):
    ...
    X = Bidirectional(GRU(p['gru_size2'], return_sequences=True, activation='relu', kernel_initializer='he_uniform'))(X)
    X = BatchNormalization()(X)
    X = Dropout(float(p['dropout2']))(X) 
                
    X = Bidirectional(GRU(p['gru_size3'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X)
    X = BatchNormalization()(X)
