...
def POC_model(input_shape_hot,DR):
    ...
    X1 = Conv1D(filters=64, kernel_size=10, strides=1, activation='relu')(X1) # 611/1 + 1 = 612
    X1 = BatchNormalization()(X1)
    X1 = Dropout(DR)(X1)

    X1 = Conv1D(filters=128, kernel_size=10, strides=1, activation='relu')(X1) # 602/1 + 1 = 603
    X1 = BatchNormalization()(X1)
    X1 = Dropout(DR)(X1)

    X1 = MaxPooling1D(pool_size=602, strides=602)(X1)
