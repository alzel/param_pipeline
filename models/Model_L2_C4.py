...
def POC_model(shapes, p):
    ...
    # actual model_l1 layers output that interests us => .layers[-1].output
    #last_layers = [model.layers[-1].output for model in models_l1]
    last_layers = X
    last_shapes = [model.layers[-1].output_shape for model in models_l1]
    
    # will need to reshape proper layer outputs
    last_layers = [Reshape([last_shapes[i][1]*last_shapes[i][2],1])(last_layers[i]) for i in range(len(last_layers))]
    ...
    X = Conv1D(filters=int(p['filters4']),kernel_size=int(p['kernel_size4']), strides=1, activation='relu', kernel_initializer='he_uniform')(X)
    X = BatchNormalization()(X)
    X = Dropout(float(p['dropout4']))(X)
    X = MaxPooling1D(pool_size=int(p['pool_size4']), strides=int(p['stride4']), padding='same')(X)
