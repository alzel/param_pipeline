...
def POC_model(shapes, p):
    ...
    X = Bidirectional(GRU(p['gru_size1'], return_sequences=True, activation='relu', kernel_initializer='he_uniform'))(X)
    X = BatchNormalization()(X)
    X = Dropout(float(p['dropout1']))(X) 
                
    X = Bidirectional(GRU(p['gru_size2'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X)
    X = BatchNormalization()(X)
