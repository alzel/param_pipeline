...
def Params():
    ...
    params = {
        ...
        'dropout5': (0, 1),
        'dropout6': (0, 1),
        'dense5': [32, 64, 128, 256],
        'dense6': [32, 64, 128, 256]
    }
...
#     X = Concatenate(axis=1)(last_layers)
    ...
    X1 = Bidirectional(GRU(p['gru_size1'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X[0])
    X1 = BatchNormalization()(X1)
    X1 = Dropout(float(p['dropout1']))(X1)
    
    X2 = Bidirectional(GRU(p['gru_size2'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X[1])
    X2 = BatchNormalization()(X1)
    X2 = Dropout(float(p['dropout2']))(X1)
    
    X3 = Bidirectional(GRU(p['gru_size3'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X[2])
    X3 = BatchNormalization()(X3)
    X3 = Dropout(float(p['dropout3']))(X3)
    
    X4 = Bidirectional(GRU(p['gru_size4'], return_sequences=False, activation='relu', kernel_initializer='he_uniform'))(X[3])
