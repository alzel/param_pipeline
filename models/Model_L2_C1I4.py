...
def POC_model(shapes, p):
    ...
    X = Conv1D(filters=int(p['filters1']),kernel_size=int(p['kernel_size1']),strides=1, activation='relu', kernel_initializer='he_uniform')(X)
    ...
    X1 = Conv1D(filters=int(p['filters2']),kernel_size=10, strides=1, padding='same', activation='relu', kernel_initializer='he_uniform')(X)
    X1 = BatchNormalization()(X1)
    X1 = Dropout(float(p['dropout2']))(X1)
    X1 = MaxPooling1D(pool_size=int(p['pool_size2']), strides=int(p['stride2']), padding='same')(X1)

    X2 = Conv1D(filters=int(p['filters3']),kernel_size=20, strides=1, padding='same', activation='relu', kernel_initializer='he_uniform')(X)
    X2 = BatchNormalization()(X2)
    X2 = Dropout(float(p['dropout3']))(X2)
    X2 = MaxPooling1D(pool_size=int(p['pool_size3']), strides=int(p['stride3']), padding='same')(X2)
    
    X3 = Conv1D(filters=int(p['filters4']),kernel_size=30, strides=1, padding='same', activation='relu', kernel_initializer='he_uniform')(X)
    X3 = BatchNormalization()(X3)
    X3 = Dropout(float(p['dropout4']))(X3)
    X3 = MaxPooling1D(pool_size=int(p['pool_size4']), strides=int(p['stride4']), padding='same')(X3)
    
    X4 = Conv1D(filters=int(p['filters5']),kernel_size=40, strides=1, padding='same', activation='relu', kernel_initializer='he_uniform')(X)
    X4 = BatchNormalization()(X4)
    X4 = Dropout(float(p['dropout5']))(X4)
    X4 = MaxPooling1D(pool_size=int(p['pool_size5']), strides=int(p['stride5']), padding='same')(X4)
