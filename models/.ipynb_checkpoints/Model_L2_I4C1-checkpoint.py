...
def POC_model(shapes, p):
    ...
    X = Conv1D(filters=int(p['filters5']),kernel_size=int(p['kernel_size5']), strides=1, activation='relu', kernel_initializer='he_uniform')(X)
    X = BatchNormalization()(X)
    X = Dropout(float(p['dropout5']))(X)
    X = MaxPooling1D(pool_size=int(p['pool_size5']), strides=int(p['stride5']), padding='same')(X)
